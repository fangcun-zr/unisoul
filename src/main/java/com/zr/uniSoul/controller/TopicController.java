...
public class TopicController {
    ...
    public R<List<Replies>> getReplies(HttpServletRequest request,@RequestParam Long topicId){
        return R.success(topicService.getReplies(topicId));
    }
    ...
    public R<String> deleteReplies(HttpServletRequest request,@RequestParam Long id){
        String username = (String)request.getSession().getAttribute("username");
        if(username == null){
            return R.error("用户未登录");
        }
        boolean judge =  topicService.deleteReplies(username,id);
        if(judge){
            return R.success("删除成功");
        }else{
            return R.error("删除失败");
    ...
    public R<List<Topic>> getTopicsByTags(@RequestParam(value = "tagsName") List<String> tags){
        log.info(tags.toString());
        return R.success(topicService.getTopicsByTags(tags));
    }
    ...
    public R<List<Topic>> searchKeyWord(@RequestParam(value = "keyWord") String keyWord){
        log.info("关键词查询，{}",keyWord);
        return R.success(topicService.searchKeyWord(keyWord));
    }
